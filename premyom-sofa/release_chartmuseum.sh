...
CHART_DIR="${REPO_DIR}/premyom-sofa"
...
IMAGE_NAME="${IMAGE_NAME:-onyxia-sofa}"
SOFA_VERSION="${SOFA_VERSION:-25.12.00}"
SOFA_DOWNLOAD_URL="${SOFA_DOWNLOAD_URL:-}"
...
TARBALL="premyom-sofa-${CHART_VERSION}.tgz"
...
cleanup() {
  ...
  if [ "${RESTORE_CHART_FILES:-0}" = "1" ]; then
    git -C "${REPO_DIR}" checkout -- \
      "premyom-sofa/values.yaml" \
      "premyom-sofa/values.schema.json" \
      "premyom-sofa/Chart.yaml" >/dev/null 2>&1 || true
  fi
}
...
echo "[INFO] chart: premyom-sofa:${CHART_VERSION}"
...
grep -n "name: SOFA_WEB_DEFAULT_PATH" "${CHART_DIR}/templates/deployment.yaml"
grep -n "name: SOFA_XPRA_DESKTOP_SCALING" "${CHART_DIR}/templates/deployment.yaml"
grep -n "desktop_scaling=auto" "${CHART_DIR}/templates/deployment.yaml" "${CHART_DIR}/templates/oauth2-proxy-redirect-middleware.yaml"
...
docker run --rm --entrypoint /bin/bash "${IMAGE_REF}" -lc \
  'test -x /opt/sofa/bin/runSofa && test -x /usr/local/bin/runSofa && command -v xpra >/dev/null && su -s /bin/bash -c "sudo -n true && echo sudo-nopasswd=OK" onyxia'
...
(
  TMP_DIR="$(mktemp -d)"
  cp -a "${CHART_DIR}" "${TMP_DIR}/premyom-sofa"
  cd "${TMP_DIR}"
  helm package premyom-sofa --version "${CHART_VERSION}" --app-version "${CHART_APP_VERSION}"
...
grep -n "repository: ${IMAGE_REGISTRY_HOST}/${IMAGE_NAMESPACE}/${IMAGE_NAME}" "${TMP_DIR}/premyom-sofa/values.yaml"
grep -n "tag: ${IMG_TAG}" "${TMP_DIR}/premyom-sofa/values.yaml"
grep -n "\"default\": \"${IMAGE_REGISTRY_HOST}/${IMAGE_NAMESPACE}/${IMAGE_NAME}\"" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "\"default\": \"${IMG_TAG}\"" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "^version: ${CHART_VERSION}$" "${TMP_DIR}/premyom-sofa/Chart.yaml"
grep -n "resizeMode" "${TMP_DIR}/premyom-sofa/values.yaml" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "autoconnect" "${TMP_DIR}/premyom-sofa/values.yaml" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "name: SOFA_WEB_DEFAULT_PATH" "${TMP_DIR}/premyom-sofa/templates/deployment.yaml"
grep -n "name: SOFA_XPRA_DESKTOP_SCALING" "${TMP_DIR}/premyom-sofa/templates/deployment.yaml"
grep -n "desktop_scaling=auto" "${TMP_DIR}/premyom-sofa/templates/deployment.yaml" "${TMP_DIR}/premyom-sofa/templates/oauth2-proxy-redirect-middleware.yaml"
grep -n "oauth.apps.datalab.arkam-group.com" "${TMP_DIR}/premyom-sofa/values.yaml" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "\.apps.datalab.arkam-group.com" "${TMP_DIR}/premyom-sofa/values.yaml" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "apps.{{k8s.domain}}" "${TMP_DIR}/premyom-sofa/values.schema.json"
grep -n "X-Auth-Request-Redirect" "${TMP_DIR}/premyom-sofa/templates/oauth2-proxy-redirect-middleware.yaml"

echo "[STEP] validating Xpra source markers"
grep -n "xpra" "${CHART_DIR}/image/Dockerfile"
grep -n "Xpra HTML5 client detected" "${CHART_DIR}/image/onyxia-init.sh"
grep -n "exec xpra start" "${CHART_DIR}/image/onyxia-init.sh"
grep -n -- "--html=on" "${CHART_DIR}/image/onyxia-init.sh"
grep -n -- "--bind-tcp=0.0.0.0:8080" "${CHART_DIR}/image/onyxia-init.sh"
grep -n -- "--tcp-auth=none" "${CHART_DIR}/image/onyxia-init.sh"
grep -n "premyom-sofa-xpra-pasteboard-fix" "${CHART_DIR}/image/onyxia-init.sh"
grep -n "premyom-sofa-xpra-clipboard-guard" "${CHART_DIR}/image/onyxia-init.sh"

echo "[STEP] validating Xpra packaged markers"
grep -n "xpra" "${TMP_DIR}/premyom-sofa/image/Dockerfile"
grep -n "Xpra HTML5 client detected" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n "exec xpra start" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n -- "--html=on" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n -- "--bind-tcp=0.0.0.0:8080" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n -- "--tcp-auth=none" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n "premyom-sofa-xpra-pasteboard-fix" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
grep -n "premyom-sofa-xpra-clipboard-guard" "${TMP_DIR}/premyom-sofa/image/onyxia-init.sh"
...
curl -fsSL "${CHARTMUSEUM_URL%/}/index.yaml" | grep -n "premyom-sofa-${CHART_VERSION}.tgz"

cat <<EOFMSG
[DONE] release published.
POC CPU-only (SOFA web desktop) - next commands (arkam-master):
