...
sed -i.bak -E "/\"repository\": *\\{/{ s#(\"default\": \")[^\"]+(\",)#\\1${IMAGE_REGISTRY_HOST}/${IMAGE_NAMESPACE}/${IMAGE_NAME}\\2#; }" "${CHART_DIR}/values.schema.json"
sed -i.bak -E "/\"tag\": *\\{/{ s#(\"default\": \")[^\"]+(\",)#\\1${IMG_TAG}\\2#; }" "${CHART_DIR}/values.schema.json"
