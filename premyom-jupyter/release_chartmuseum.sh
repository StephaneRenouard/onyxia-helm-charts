...
docker run --rm --entrypoint /bin/bash "${IMAGE_REF}" -lc \
  'python3.12 --version && source /opt/conda/etc/profile.d/conda.sh && conda --version && jupyter lab --version && R --version | head -n1 | grep -E "^R version 4\\.5\\." && jupyter kernelspec list | grep -q ir && nano --version | head -n1 && su -s /bin/bash -c "sudo -n true && echo sudo-nopasswd=OK" onyxia'
...
grep -n "oauth.apps.datalab.arkam-group.com" "${TMP_DIR}/premyom-jupyter/values.yaml" "${TMP_DIR}/premyom-jupyter/values.schema.json"
grep -n "\.apps.datalab.arkam-group.com" "${TMP_DIR}/premyom-jupyter/values.yaml" "${TMP_DIR}/premyom-jupyter/values.schema.json"
grep -n "apps.{{k8s.domain}}" "${TMP_DIR}/premyom-jupyter/values.schema.json"
grep -n "X-Auth-Request-Redirect" "${TMP_DIR}/premyom-jupyter/templates/oauth2-proxy-redirect-middleware.yaml"
